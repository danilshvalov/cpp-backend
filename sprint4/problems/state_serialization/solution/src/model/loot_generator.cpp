...
unsigned LootGenerator::Generate(
    ...
    const double ratio = std::chrono::duration<double> {time_without_loot_} /
        config_.base_interval;
    const double probability = std::clamp(
        (1.0 - std::pow(1.0 - config_.probability, ratio)) *
            random_generator_(),
        0.0,
        1.0
    );
