...
SCENARIO("TV", "[TV]") {
    GIVEN("A TV") {  // Дано: Телевизор
        ...
        // Когда его включают в первый раз,
        WHEN("it is turned on first time") {
            ...
            AND_THEN("it can select channel from 1 to 99") {
                for (int channel = 1; channel <= 99; ++channel) {
                    CHECK_NOTHROW(tv.SelectChannel(channel));
                }
            }
            AND_THEN("check channel out of range") {
                CHECK_THROWS_AS(
                    tv.SelectChannel(tv.MIN_CHANNEL - 1),
                    std::out_of_range
                );
                CHECK_THROWS_AS(
                    tv.SelectChannel(tv.MAX_CHANNEL + 1),
                    std::out_of_range
                );
            }
        }
...
SCENARIO("Controller", "[Controller]") {
    ...
    GIVEN("Controller and TV") {
        ...
        WHEN("The TV is turned off") {
            ...
            AND_THEN("SelectChannel command is entered") {
                run_menu_command("SelectChannel 12");

                THEN("the error message is printed and channel is not selected"
                ) {
                    CHECK(tv.GetChannel() == std::nullopt);
                    expect_output("TV is turned off\n");
                }
            }
            AND_THEN("SelectPreviousChannel command is entered") {
        ...
        WHEN("The TV is turned on") {
            ...
            /* TODO: Протестируйте остальные аспекты поведения класса Controller, когда TV включен */
            AND_WHEN("SelectChannel command is entered") {
                run_menu_command("SelectChannel 12");

                THEN("current channel is printed") {
                    CHECK(tv.GetChannel() == 12);
                    expect_output("Channel number is 12\n");
                }
            }
            ...
            AND_WHEN("SelectChannel command is entered multiply arguments") {
                run_menu_command("SelectChannel 12 13");
            ...
            AND_WHEN(
                "SelectChannel command is entered with out of range channel"
            ) {
                run_menu_command(
                    "SelectChannel " + std::to_string(tv.MAX_CHANNEL + 1)
                );
            ...
            AND_WHEN("SelectChannel command is entered with float channel") {
                run_menu_command("SelectChannel 1.1");
            ...
            AND_WHEN("SelectPreviousChannel command is entered") {
                ...
                THEN("current channel is printed") {
                    CHECK(tv.GetChannel() == 1);
                    expect_output("Channel number is 1\n");
                }
            }
            AND_WHEN(
                "SelectPreviousChannel command is entered with some arguments"
            ) {
                run_menu_command("SelectPreviousChannel some args");

                THEN("the error message is printed and TV is not turned off") {
                    expect_extra_arguments_error("SelectPreviousChannel");
                }
            }
        }
    }
